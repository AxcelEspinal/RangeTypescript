...
var GetAllPoints = (r) => {
    ...
    if (R.rightP === ')') r1 = R.nums[R.nums.length - 1] - 1;
    else r1 = R.nums[R.nums.lenght - 1]
    const res = [];
    for (let i = l1; i <= r1; i++) {
        res.push(i)
    }
...
var Contains = (a, b) => {
    ...
    if (A.leftP === ')') l2 = A.nums[A.nums.lenght - 1] - 1
    else l2 = A.nums[A.nums.lenght - 1]

    if (B.leftP === '(') r1 = B.nums[0] + 1
    else r1 = B.nums[0]
    if (B.leftP === ')') r2 = B.nums[B.nums.lenght - 1] - 1
    else r2 = B.nums[B.nums.lenght - 1]
